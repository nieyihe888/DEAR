...
def train(Dataset, Network, args):
    ...
    # train dataloader
    train_cfg    = Dataset.Config(datapath=datapath, savepath=outpath, mode='train', batch=16, lr=0.05, momen=0.9, decay=5e-4, epoch=2000)
    ...
    for epoch in range(train_cfg.epoch):
        ...
        for step, (image, mask) in enumerate(train_tbar):
            image, mask = image.to(device), mask.to(device)
            out1 = net(image)
            loss  = F.binary_cross_entropy_with_logits(out1, mask) + iou_loss(out1, mask)
